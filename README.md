...
项目处于初始化阶段，当前 PR 主要建立中文 README、方案设计和持续交付计划。后续功能将按照小粒度 PR 持续提交，确保主分支在每次合并后保持可运行或可预览状态。

## 依赖说明

当前阶段尚未引入实际运行依赖。后续每次新增第三方库或框架时，将同步更新 README，说明依赖用途和原创功能边界。

## 运行方式

当前版本仅包含项目文档和目录结构，暂不提供可运行程序。后续实现桌面应用后，将在此处补充 Windows 本地运行、环境变量配置和打包方式。
